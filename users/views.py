    ...
    def post(self, request):
        try:
            ...
            access_token = requests.post(
                f"https://github.com/login/oauth/access_token?code={code}&client_id=f6466eae624ca07866e2&client_secret={settings.GH_SECRET}",
    ...
    def post(self, request):
        try:
            ...
            if check_username or check_email:
                return Response(
                    {"duplicated": "email and username are already exist"},
                    status=status.HTTP_409_CONFLICT,
                )
            elif check_username:
                return Response(
                    {"duplicated": "username is already exist"},
                    status=status.HTTP_409_CONFLICT,
                )
            elif check_email:
                return Response(
                    {"duplicated": "email is already exist"},
                    status=status.HTTP_409_CONFLICT,
                )
